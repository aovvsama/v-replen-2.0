...
function updateResultsTable() {
    ...
    filteredList.forEach((item, index) => {
        ...
        const priorityClass = `priority-${item.priority === '高' ? 'high' : item.priority === '中' ? 'medium' : 'low'}`;
    ...
    try {
        ...
        const wsData = [
            ...
            [
                '状态',
                'ProductGender',
                'Color Choice',  // 原Item Number，现改为Color Choice
                'Product name',
        ...
        const wscols = [
            ...
            { wch: 25 },  // Color Choice（原Item Number，现在包含Item Number和Color）
            { wch: 35 },  // Product name
            { wch: 8 },   // Size
            { wch: 12 },  // Order Qty
            { wch: 12 },  // 当前库存
